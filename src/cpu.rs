use crate::{float_to_bigint, mul_bigint, CalculateEscapeTimes, RenderingState};
use num::BigInt;
    ...
    fn calculate_escape_times(&self, state: &RenderingState) -> Vec<u32> {
        ...
        if state.screen_width > state.screen_height {
            width = mul_bigint(
                &width,
                &float_to_bigint(
                    state.screen_width as f64 / state.screen_height as f64,
                    state.digits,
                ),
                state.digits,
            )
            ...
            height = mul_bigint(
                &height,
                &float_to_bigint(
                    state.screen_height as f64 / state.screen_width as f64,
                    state.digits,
                ),
                state.digits,
            );
        ...
        let mut escape_times =
            vec![0_u32; state.screen_width as usize * state.screen_height as usize];
        ...
        for y in 0..state.screen_height as usize {
            for x in 0..state.screen_width as usize {
                ...
                for i in 0..state.max_iterations {
                    let new_z_real = mul_bigint(&z_real, &z_real, state.digits)
                        - mul_bigint(&z_img, &z_img, state.digits)
                        + c_real.clone();
                    z_img = (mul_bigint(&z_real, &z_img, state.digits) << 1) + c_img.clone();
                    z_real = new_z_real;

                    if mul_bigint(&z_real, &z_real, state.digits)
                        + mul_bigint(&z_img, &z_img, state.digits)
                        > float_to_bigint(4.0, state.digits)
                    {
